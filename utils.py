...
def createSubdiv2D(size, landmarks):
    ...
    for i in landmarks:
        newLandmarks.append((int(i[0]), int(i[1])))
...
def getLips(points):
    ...
    if inFlag: 
        # Insert points into subdiv
        for i in range(0, len(innerLipsIndex)) :
            subdiv_new.insert((int(points_inLips[i,0]), int(points_inLips[i,1])))     
        ...
        # Insert points into subdiv
        for i in range(0, len(outerLipsIndex)) :
            subdiv_new.insert((int(points_outLips[i,0]), int(points_outLips[i,1])))     
        # get the initial indices of the points in lips triangulation
